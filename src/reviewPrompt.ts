 ...
 * a real number of distinct leftover conflict markers flagged -- never on install, never on
...
const KEY_SEEN_FINDINGS = 'mergeConflictLeftoverCompanion.review.seenFindings';
const KEY_ANSWERED = 'mergeConflictLeftoverCompanion.review.answered';
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Merge Conflict Leftover Companion: this has flagged ${HITS_BEFORE_PROMPT} real issues in your code -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
